  ...
  return (
    <div className="w-full">
      <div className="relative border-b-2 border-slate-100 overflow-x-auto">
        <nav className="flex gap-2 w-max sm:w-full px-2">
          {tabs.map((tab) => (
            <button
              key={tab.name}
              onClick={() => setActiveTab(tab.name)}
              className={`relative pb-3 sm:pb-4 px-4 sm:px-6 text-sm sm:text-base font-semibold transition-all duration-200 whitespace-nowrap ${
                activeTab === tab.name
                  ? "text-emerald-600"
